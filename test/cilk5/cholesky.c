...
const char *specifiers[] = { "-n", "-z", "-c", "-f", "-benchmark", "-h", "-r", 0 };
...
int main (int argc, char *argv[]) {
  ...
  Matrix A, R;
  int size, depth, nonzeros, i, benchmark, help, check = 1, round = 1;
  ...
  get_options(argc, argv, specifiers, opt_types,
	      &size, &nonzeros, &check, filename, &benchmark, &help, &round);
